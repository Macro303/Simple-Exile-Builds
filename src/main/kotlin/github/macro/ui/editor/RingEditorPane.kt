 ...
 */
class RingEditorPane(val model: UIModel, ring: Ring, val index: Int? = null) : BorderPane() {
	...
	companion object {
		private val LOGGER = LogManager.getLogger(RingEditorPane::class.java)
	}
...
//							assignRing(//TODO)
				...
				row {
					button("➕") {
						useMaxWidth = true
						visibleWhen((nextProperty.or(ringProperty.isEqualTo(Data.MISSING_RING)).not()))
						action {
							LOGGER.info("Selected Add Ring")
						}
						gridpaneConstraints {
							margin = Insets(2.5)
						}
					...
					button("⚙") {
						useMaxWidth = true
						action {
							LOGGER.info("Selected Edit Ring")
						}
						gridpaneConstraints {
							margin = Insets(2.5)
						}
					...
					button("✖") {
						...
						action {
							LOGGER.info("Selected Delete Ring")
						}
						gridpaneConstraints {
							margin = Insets(2.5)
						}
