package github.macro.core.item.gear.amulet
...
class BuildAmulet(
	gear: Amulet,
	nextGear: BuildAmulet? = null,
	reason: String? = null,
	preferences: List<String?> = mutableListOf()
) : BaseBuildGear(gear = gear, nextGear = nextGear, reason = reason)
...
private class BuildAmuletDeserializer @JvmOverloads constructor(vc: Class<*>? = null) :
	...
	override fun deserialize(parser: JsonParser, ctx: DeserializationContext?): BuildAmulet {
		...
		val nextGear = if (node["Next Gear"]?.isNull != false)
			...
			Util.YAML_MAPPER.treeToValue(node["Next Gear"], BuildAmulet::class.java)
